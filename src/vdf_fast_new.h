...
template<class int1x, class int2x, class int3x, class int4x> struct square_state_type {
    ...
    int1x int1x_buffer_master;
    int4x int4x_buffer_slave;
    ...
    struct phase_1_master_d_type {
        ...
        bool S_negative() { return v1().is_negative_nonzero(); }
    } phase_1_master_d;
    ...
    bool phase_0_master() {
        ...
        {
            TRACK_CYCLES //185
            ab_valid=(a.num_bits()<=max_bits_ab && b.num_bits()<=max_bits_ab && !a.is_negative_nonzero());
        }
        if (!ab_valid) {
            return false;
        }
    ...
    bool phase_0_slave() {
...

        {
            ...
            {
                TRACK_CYCLES //100
                if (a.is_negative_nonzero() || c.is_negative_nonzero()) {
                    assert(!is_vdf_test);
        ...
        while (true) {
            ...
            if (gcd_index!=0) {
                ...
                {
                    TRACK_CYCLES //325
                    c_entry->matrix_multiply(in_U0, in_U1, out_U0, out_U1, int4x_buffer_slave);
                    ++num_multiplications;
                    mod_pending=true;
                }
    ...
    bool phase_1_master() {
        ...
        while (true) {
            ...
            if (gcd_index!=0) {
                ...
                c_entry->matrix_multiply(in_v0, in_v1, out_v0, out_v1, int1x_buffer_master);
    ...
    bool phase_4_master() {
...

        {
            ...
            if (gcd_1_0.get_b_end().is_nonzero()) {
                assert(!is_vdf_test);
                phase_start.corruption_flag=true;
                return false;
            }
